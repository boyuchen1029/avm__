...
//float h3_dotvector3x3(float *a, float *b);
...
static int save_to_file(int ch, struct video_channel *video_ch)
{
    ...
    if (fd && !g_performance_test) {
        ...
        for (i = 0; i < g_num_planes; i++) {
            wsize_save = write(fd, video_ch[ch].buffers[buf_id].planes[i].start,
                    video_ch[ch].buffers[buf_id].planes[i].plane_size);
            printf("video_ch[ch].buffers[buf_id].planes[i].plane_size = %d\n", video_ch[ch].buffers[buf_id].planes[i].plane_size);
            if (wsize_save < 1) {
...
static int deQdraw(struct media_dev *media)
{
    struct video_channel *video_ch = media->v4l2_dev->video_ch;
    int i, ret;
...
///     ret = display_on_screen(0, media, resultResult);
        // if (video_ch[0].on) {
        //  display_on_screen(0, media, bufferFunc);
        // }
...
void init_v4l2(int argc, const char *argv[], v4l2_status_t *v4l2Status)
{

#ifndef READ_FILE
    printf("GenTextures success \n");
    ...
    if (ret < 0)
        *v4l2Status = V4L2_STATUS_OPEN_FAIL;

//      goto free;
    //v4l2_info("!!!!!!!!!!!!!media_device_prepare\n");


    ret = media_device_prepare(&media);
    if (ret < 0)
        *v4l2Status = V4L2_STATUS_PREPARE_FAIL;
//      goto close;
    //v4l2_info("!!!!!!!!!!!!!media_device_start\n");
    ret = media_device_start(&media);
    if (ret < 0)
        *v4l2Status = V4L2_STATUS_START_FAIL;
//      goto cleanup;
    //struct video_channel *video_ch = media->v4l2_dev->video_ch;
    //fp_writeoutF = fopen("outcamF.yuv","wb");
    //fp_writeoutB = fopen("outcamB.yuv","wb");
    // fp_writeoutL = fopen("outcamL.yuv","wb");
    // fp_writeoutR = fopen("outcamR.yuv","wb");
    //Camera init
#endif
}

void end_avm(v4l2_status_t *v4l2Status)
{

    if(*v4l2Status == V4L2_STATUS_ROUTINE_FAIL)
    {
...
//stop:
    ...
    if( *v4l2Status == V4L2_STATUS_OPEN_FAIL ||
        *v4l2Status == V4L2_STATUS_PREPARE_FAIL ||
        *v4l2Status == V4L2_STATUS_START_FAIL)
    {
//  cleanup:
        if(*v4l2Status == V4L2_STATUS_START_FAIL)
        {
...
void get_camera_buf(unsigned char ** cameraFrontBuf,
                     ...
                     unsigned char ** cameraBackBuf)
{
    ...
    *cameraFrontBuf = (unsigned char *)video_ch[0].buffers[buf_id].planes[0].start;
    *cameraLeftBuf = (unsigned char *)video_ch[1].buffers[buf_id].planes[0].start;
    *cameraRightBuf = (unsigned char *)video_ch[2].buffers[buf_id].planes[0].start;
    *cameraBackBuf = (unsigned char *)video_ch[3].buffers[buf_id].planes[0].start;
}
*/
...
{
    cameraFBufVA = cameraBufF;
    cameraBBufVA = cameraBufB;
    cameraLBufVA = cameraBufL;
    cameraRBufVA = cameraBufR;
}
